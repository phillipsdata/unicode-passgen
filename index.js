...
(function() {
  ...
  module.exports = {
        ...
        for (var set in options[property]) {
          if (typeof options[property][set] === 'object'
            ...
            if (options[property][set].hasOwnProperty('min')
              && this.isInt(options[property][set]['min'])
    ...
    generate: function(length, options) {
      ...
      var allCharacters = this.getCharacterList(opts);
      ...
      var characterSets = this.getCharacterSets(opts);
      for (var i in characterSets) {
        value += this.generateString(characterSets[i]['set'], characterSets[i]['min']);
      }

      // Generate random characters for the remainder of the length
      value += this.generateString(allCharacters, (length - value.length));
     ...
     */
    generateString: function(set, length) {
      var content = '';
     ...
     */
    getCharacterList: function(options) {
      ...
      for (var i in include) {
        for (var j in include[i]['chars']) {
          ...
          regen = this.addCharacters(regen, include[i]['chars'][j]);
        }
      }

      this.removeCharacters(regen, options.exclude);
     ...
     */
    getCharacterSets: function(options) {
      var regen;
      var include = options.include;
      var sets = [];
      ...
      for (var i in include) {
        ...
        regen = this.removeCharacters(regen, options.exclude);
     ...
     */
    addCharacters: function(regen, set) {
     ...
     * @returns {Regenerate} An updated instance of the Regenerate object
     */
    removeCharacters: function(regen, set) {
